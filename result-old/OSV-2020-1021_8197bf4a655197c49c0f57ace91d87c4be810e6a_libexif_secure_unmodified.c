exif_entry_get_value (ExifEntry *e, char *val, unsigned int maxlen /* non attacker controlled value: 1024 */)
{
	unsigned int i, j, k;
	...
	switch (e->tag) {
	case EXIF_TAG_USER_COMMENT: //?!! enum const EXIF_TAG_USER_COMMENT = 37510
		 ...
		 */
		if ((e->format != EXIF_FORMAT_ASCII) ||  //?!! enum const EXIF_FORMAT_ASCII = 2
		    (e->size <= 8) ||
		    ( memcmp (e->data, "ASCII\0\0\0"  , 8) &&
		      memcmp (e->data, "UNICODE\0"    , 8) &&
		      memcmp (e->data, "JIS\0\0\0\0\0", 8) &&
		      memcmp (e->data, "\0\0\0\0\0\0\0\0", 8)))
			CF (e, EXIF_FORMAT_UNDEFINED, val, maxlen); //?!! enum const EXIF_LOG_CODE_CORRUPT_DATA = 3 //?!! enum const EXIF_FORMAT_UNDEFINED = 7
		 ...
		 */
		exif_entry_log (e, EXIF_LOG_CODE_DEBUG, //?!! enum const EXIF_LOG_CODE_DEBUG = 1
	...
	case EXIF_TAG_EXIF_VERSION: //?!! enum const EXIF_TAG_EXIF_VERSION = 36864
		...
		break;
	case EXIF_TAG_FLASH_PIX_VERSION: //?!! enum const EXIF_TAG_FLASH_PIX_VERSION = 40960
		CF (e, EXIF_FORMAT_UNDEFINED, val, maxlen);
		CC (e, 4, val, maxlen);
		...
		break;
	case EXIF_TAG_COPYRIGHT: //?!! enum const EXIF_TAG_COPYRIGHT = 33432
		...
		break;
	case EXIF_TAG_FNUMBER: //?!! enum const EXIF_TAG_FNUMBER = 33437
		CF (e, EXIF_FORMAT_RATIONAL, val, maxlen); //?!! enum const EXIF_FORMAT_RATIONAL = 5
		CC (e, 1, val, maxlen);
		...
		break;
	case EXIF_TAG_APERTURE_VALUE: //?!! enum const EXIF_TAG_APERTURE_VALUE = 37378
	case EXIF_TAG_MAX_APERTURE_VALUE: //?!! enum const EXIF_TAG_MAX_APERTURE_VALUE = 37381
		...
		break;
	case EXIF_TAG_FOCAL_LENGTH: //?!! enum const EXIF_TAG_FOCAL_LENGTH = 37386
		CF (e, EXIF_FORMAT_RATIONAL, val, maxlen);
		CC (e, 1, val, maxlen);
		v_rat = exif_get_rational (e->data, o);
		if (!v_rat.denominator) {
		...
		entry = exif_content_get_entry (
			e->parent->parent->ifd[EXIF_IFD_0], EXIF_TAG_MAKE); //?!! enum const EXIF_TAG_MAKE = 271 //?!! enum const EXIF_IFD_0 = 0
		if (entry && entry->data &&
		    !strncmp ((char *)entry->data, "Minolta", 7)) {
			entry = exif_content_get_entry (
					e->parent->parent->ifd[EXIF_IFD_0],
					EXIF_TAG_MODEL); //?!! enum const EXIF_TAG_MODEL = 272
			if (entry && entry->data) {
		...
		strncat (val, b, maxlen-1 - strlen (val));
		break;
	case EXIF_TAG_SUBJECT_DISTANCE: //?!! enum const EXIF_TAG_SUBJECT_DISTANCE = 37382
		CF (e, EXIF_FORMAT_RATIONAL, val, maxlen);
		CC (e, 1, val, maxlen);
		v_rat = exif_get_rational (e->data, o);
		if (!v_rat.denominator) {
		...
		break;
	case EXIF_TAG_EXPOSURE_TIME: //?!! enum const EXIF_TAG_EXPOSURE_TIME = 33434
		CF (e, EXIF_FORMAT_RATIONAL, val, maxlen);
		CC (e, 1, val, maxlen);
		v_rat = exif_get_rational (e->data, o);
		if (!v_rat.denominator) {
		...
		break;
	case EXIF_TAG_SHUTTER_SPEED_VALUE: //?!! enum const EXIF_TAG_SHUTTER_SPEED_VALUE = 37377
		CF (e, EXIF_FORMAT_SRATIONAL, val, maxlen); //?!! enum const EXIF_FORMAT_SRATIONAL = 10
		CC (e, 1, val, maxlen);
		...
		strncat (val, b, maxlen-1 - strlen (val));
		break;
	case EXIF_TAG_BRIGHTNESS_VALUE: //?!! enum const EXIF_TAG_BRIGHTNESS_VALUE = 37379
		...
		strncat (val, b, maxlen-1 - strlen (val));
		break;
	case EXIF_TAG_FILE_SOURCE: //?!! enum const EXIF_TAG_FILE_SOURCE = 41728
		CF (e, EXIF_FORMAT_UNDEFINED, val, maxlen);
		CC (e, 1, val, maxlen);
		v_byte = e->data[0];
		if (v_byte == 3)
			...
			snprintf (val, maxlen, _("Internal error (unknown "
				  "value %i)"), v_byte);
		break;
	case EXIF_TAG_COMPONENTS_CONFIGURATION: //?!! enum const EXIF_TAG_COMPONENTS_CONFIGURATION = 37121
		CF (e, EXIF_FORMAT_UNDEFINED, val, maxlen);
		CC (e, 4, val, maxlen);
		for (i = 0; i < 4; i++) {
			switch (e->data[i]) {
		...
		break;
	case EXIF_TAG_EXPOSURE_BIAS_VALUE: //?!! enum const EXIF_TAG_EXPOSURE_BIAS_VALUE = 37380
		CF (e, EXIF_FORMAT_SRATIONAL, val, maxlen);
		CC (e, 1, val, maxlen);
		v_srat = exif_get_srational (e->data, o);
		if (!v_srat.denominator) {
		...
		snprintf (val, maxlen, _("%.02f EV"), d);
		break;
	case EXIF_TAG_SCENE_TYPE: //?!! enum const EXIF_TAG_SCENE_TYPE = 41729
		CF (e, EXIF_FORMAT_UNDEFINED, val, maxlen);
		CC (e, 1, val, maxlen);
		v_byte = e->data[0];
		if (v_byte == 1)
			...
			snprintf (val, maxlen, _("Internal error (unknown "
				  "value %i)"), v_byte);
		break;
	case EXIF_TAG_YCBCR_SUB_SAMPLING: //?!! enum const EXIF_TAG_YCBCR_SUB_SAMPLING = 530
		CF (e, EXIF_FORMAT_SHORT, val, maxlen); //?!! enum const EXIF_FORMAT_SHORT = 3
		CC (e, 2, val, maxlen);
		...
		break;
	case EXIF_TAG_SUBJECT_AREA: //?!! enum const EXIF_TAG_SUBJECT_AREA = 37396
	...
	case EXIF_TAG_GPS_VERSION_ID:
		...
		CF (e, EXIF_FORMAT_BYTE, val, maxlen); //?!! enum const EXIF_FORMAT_BYTE = 1
		CC (e, 4, val, maxlen);
		v_byte = e->data[0];
		...
		break;
	case EXIF_TAG_INTEROPERABILITY_VERSION: //?!! enum const EXIF_TAG_INTEROPERABILITY_VERSION = 2
	...
	case EXIF_TAG_METERING_MODE: //?!! enum const EXIF_TAG_METERING_MODE = 37383
	case EXIF_TAG_COMPRESSION: //?!! enum const EXIF_TAG_COMPRESSION = 259
	case EXIF_TAG_LIGHT_SOURCE: //?!! enum const EXIF_TAG_LIGHT_SOURCE = 37384
	case EXIF_TAG_FOCAL_PLANE_RESOLUTION_UNIT: //?!! enum const EXIF_TAG_FOCAL_PLANE_RESOLUTION_UNIT = 41488
	case EXIF_TAG_RESOLUTION_UNIT: //?!! enum const EXIF_TAG_RESOLUTION_UNIT = 296
	case EXIF_TAG_EXPOSURE_PROGRAM: //?!! enum const EXIF_TAG_EXPOSURE_PROGRAM = 34850
	case EXIF_TAG_FLASH: //?!! enum const EXIF_TAG_FLASH = 37385
	case EXIF_TAG_SUBJECT_DISTANCE_RANGE: //?!! enum const EXIF_TAG_SUBJECT_DISTANCE_RANGE = 41996
	case EXIF_TAG_COLOR_SPACE: //?!! enum const EXIF_TAG_COLOR_SPACE = 40961
		CF (e,EXIF_FORMAT_SHORT, val, maxlen);
		CC (e, 1, val, maxlen);
		v_short = exif_get_short (e->data, o);

		/* Search the tag */
		for (i = 0; list2[i].tag && (list2[i].tag != e->tag); i++); //?!! global variable list2 (const struct <unnamed>[])
		if (!list2[i].tag) {
			snprintf (val, maxlen, _("Internal error (unknown "
				  "value %i)"), v_short);
			break;
	...
	case EXIF_TAG_PLANAR_CONFIGURATION: //?!! enum const EXIF_TAG_PLANAR_CONFIGURATION = 284
	case EXIF_TAG_SENSING_METHOD: //?!! enum const EXIF_TAG_SENSING_METHOD = 41495
	case EXIF_TAG_ORIENTATION: //?!! enum const EXIF_TAG_ORIENTATION = 274
	case EXIF_TAG_YCBCR_POSITIONING: //?!! enum const EXIF_TAG_YCBCR_POSITIONING = 531
	case EXIF_TAG_PHOTOMETRIC_INTERPRETATION: //?!! enum const EXIF_TAG_PHOTOMETRIC_INTERPRETATION = 262
	case EXIF_TAG_CUSTOM_RENDERED: //?!! enum const EXIF_TAG_CUSTOM_RENDERED = 41985
	case EXIF_TAG_EXPOSURE_MODE: //?!! enum const EXIF_TAG_EXPOSURE_MODE = 41986
	case EXIF_TAG_WHITE_BALANCE: //?!! enum const EXIF_TAG_WHITE_BALANCE = 41987
	case EXIF_TAG_SCENE_CAPTURE_TYPE: //?!! enum const EXIF_TAG_SCENE_CAPTURE_TYPE = 41990
	case EXIF_TAG_GAIN_CONTROL: //?!! enum const EXIF_TAG_GAIN_CONTROL = 41991
	case EXIF_TAG_SATURATION: //?!! enum const EXIF_TAG_SATURATION = 41993
	case EXIF_TAG_CONTRAST: //?!! enum const EXIF_TAG_CONTRAST = 41992
	case EXIF_TAG_SHARPNESS: //?!! enum const EXIF_TAG_SHARPNESS = 41994
		CF (e, EXIF_FORMAT_SHORT, val, maxlen);
		CC (e, 1, val, maxlen);
		v_short = exif_get_short (e->data, o);

		/* Search the tag */
		for (i = 0; list[i].tag && (list[i].tag != e->tag); i++); //?!! global variable list (const struct <unnamed>[])
		if (!list[i].tag) {
			snprintf (val, maxlen, _("Internal error (unknown "
				  "value %i)"), v_short);
			break;
	...
	case EXIF_TAG_XP_TITLE: //?!! enum const EXIF_TAG_XP_TITLE = 40091
	case EXIF_TAG_XP_COMMENT: //?!! enum const EXIF_TAG_XP_COMMENT = 40092
	case EXIF_TAG_XP_AUTHOR: //?!! enum const EXIF_TAG_XP_AUTHOR = 40093
	case EXIF_TAG_XP_KEYWORDS: //?!! enum const EXIF_TAG_XP_KEYWORDS = 40094
	case EXIF_TAG_XP_SUBJECT: //?!! enum const EXIF_TAG_XP_SUBJECT = 40095
	{
